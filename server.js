...
app.get('/get-comments/:articleName', function (req, res) {
   // make a select request
   // return a response with the results
   pool.query('SELECT comment.*, "user_testing".username FROM article, comment, "user_testing" WHERE article.title = $1 AND article.id = comment.article_id AND comment.user_id = "user_testing".id ORDER BY comment.timestamp DESC', [req.params.articleName], function (err, result) {
      if (err) {
          res.status(500).send(err.toString());
      } else {
          res.send(JSON.stringify(result.rows));
...
app.get('/ui/imag.jpg', function (req, res) {
  res.sendFile(path.join(__dirname, 'ui', 'imag.jpg'));
});
